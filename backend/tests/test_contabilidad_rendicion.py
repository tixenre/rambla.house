...
class TestNetting:
    def test_partes_son_pablo_tincho_rental_estudio(self):
        assert set(PARTES) == {"Pablo", "Tincho", "Rental", "Estudio"}

    def test_todo_cobrado_por_uno_reparte_a_los_demas(self):
        # Junio del ejemplo: total 200k, todo lo cobró Tincho.
        corresponde = {"Pablo": 70000, "Rental": 123000, "Tincho": 7000}
        cobrado = {"Pablo": 0, "Tincho": 200000, "Rental": 0}
        r = _netting(corresponde, cobrado, {"Pablo": 0, "Tincho": 0, "Rental": 0})

        by = _by(r["personas"])
        assert by["Tincho"]["pendiente"] == -193000  # tiene de más, debe pagar
        assert by["Pablo"]["pendiente"] == 70000
        assert by["Rental"]["pendiente"] == 123000

        sug = r["sugeridos"]
        assert {"de": "Tincho", "a": "Pablo", "monto": 70000} in sug
        assert {"de": "Tincho", "a": "Rental", "monto": 123000} in sug
        assert sum(s["monto"] for s in sug) == 193000  # cierra

    def test_cuarta_parte_estudio_participa_del_netting(self):
        # Estudio le corresponden 50k pero no cobró nada — Rental cobró de más
        ...
        corresponde = {"Pablo": 0, "Tincho": 0, "Rental": 0, "Estudio": 50000}
        cobrado = {"Pablo": 0, "Tincho": 0, "Rental": 50000, "Estudio": 0}
        r = _netting(corresponde, cobrado, {"Pablo": 0, "Tincho": 0, "Rental": 0, "Estudio": 0})
        by = _by(r["personas"])
        assert by["Estudio"]["pendiente"] == 50000
        assert by["Rental"]["pendiente"] == -50000
        assert r["sugeridos"] == [{"de": "Rental", "a": "Estudio", "monto": 50000}]

    def test_balanceado_no_sugiere_nada(self):
        corresponde = {"Pablo": 100, "Tincho": 100, "Rental": 0}
        cobrado = {"Pablo": 100, "Tincho": 100, "Rental": 0}
        r = _netting(corresponde, cobrado, {"Pablo": 0, "Tincho": 0, "Rental": 0})
        assert r["sugeridos"] == []
    ...
    def test_ya_transferido_descuenta_lo_pendiente(self):
        ...
        ya = {"Pablo": 70000, "Tincho": -70000, "Rental": 0}
        r = _netting(corresponde, cobrado, ya)
        by = _by(r["personas"])
        assert by["Pablo"]["pendiente"] == 0
        # Falta saldar solo lo de Rental.
        assert r["sugeridos"] == [{"de": "Tincho", "a": "Rental", "monto": 123000}]

    def test_conservacion(self):
        corresponde = {"Pablo": 60000, "Rental": 30000, "Tincho": 10000}
        cobrado = {"Pablo": 50000, "Tincho": 50000, "Rental": 0}
        r = _netting(corresponde, cobrado, {"Pablo": 0, "Tincho": 0, "Rental": 0})
    ...
    def test_determinismo(self):
        corresponde = {"Pablo": 70000, "Rental": 123000, "Tincho": 7000}
        cobrado = {"Pablo": 0, "Tincho": 200000, "Rental": 0}
        a = _netting(corresponde, cobrado, {"Pablo": 0, "Tincho": 0, "Rental": 0})
        b = _netting(corresponde, cobrado, {"Pablo": 0, "Tincho": 0, "Rental": 0})
        assert a == b
