...
| ¿Quién le debe a quién entre las 4 partes, AL DÍA DE HOY? | `backend/contabilidad/` | `queries/posiciones.py::calcular_posiciones` (+ `clasificar_flujo`) — desde 2026-08 parte el número en `repartible` (solo pedidos cerrados) y `en_curso` (cobros de pedidos abiertos); las transferencias sugeridas salen de `repartible`, y un socio humano nunca aparece como pagador sugerido | pantalla **Finanzas** (sección "Para repartir hoy"; las viejas Cuentas/Rendición redirigen ahí), chequeo `cc_vs_posicion_divergente` |
...
- **`backend/contabilidad/`** (`CLAUDE.md` propio) — la plata interna: cajas, cuenta corriente de
  ...
  con su propia caja real (`Caja Estudio`, `tipo='fondo'`) — mismo mecanismo que Rental/Fondo Rental,
